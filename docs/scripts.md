...
- 豁免 14 码（设泛型覆盖无效）：cn01/cn03（批量 legacy 旗标驱动）、cn02（仅 nodes 有效，无 limit 概念，补测由 cn01 失败触发）、cn20-29（L2 常开全池，无条数概念）、cn41（搭 cn40 复核相，需 token）。R99 起对豁免码的覆盖打 stderr warn（对应泛型覆盖参数对该代号无效），无包时跳过此提示。
